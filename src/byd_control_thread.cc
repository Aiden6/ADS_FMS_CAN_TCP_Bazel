...
void* acu_ros_Thread( void *data )
{
	...
	ros::Subscriber sub = nh_sub.subscribe("/apollo/control", 1000, chatterCallback);	//向话题“str_message”订阅，一旦发布节点（node_a）在该话题上发布消息，本节点就会调用chatterCallbck函数。
 	ros::Publisher pub = nh_pub.advertise<apollo::canbus::Chassis>("/apollo/canbus/chassis", 1000);	//告诉系统要发布话题了，话题名为“str_message”，类型为std_msgs::String，缓冲队列为1000。
...
while (gTestFlag) {
	while(ros::ok())
	{
        ...
        chassis.set_speed_mps(realSpeed_val);
        chassis.set_steering_percentage(realAngleInfo_val);
        chassis.set_brake_percentage(brakeDeepness_val);
        //chassis.set_gear_location(carGear);
        chassis.set_throttle_percentage(accDeepness_val);
/*         chassis.set_speed_mps(1);
        chassis.set_steering_percentage(1);
        chassis.set_brake_percentage(1); */
        pub.publish(chassis);	//向话题“str_message”发布消息
...
{
    AccVal = control_cmd.throttle();
    sem_post(&sem_event_SetAcceleratedVelocity);
    nAngular = (-1)*control_cmd.steering_target();
...
void* acu_JudgeAutoDriveExit_thread( void *data )
{
    ...
    while (gTestFlag) {
        ...
        if((accDeepness_val > 10)||(brakeDeepness_val > 10)||(brakePedalStatus == BYD_AUTO_BRAKE_PEDAL_SIGNAL_PRESSED)){
